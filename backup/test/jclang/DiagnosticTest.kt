...
class DiagnosticTest : StringSpec({
    ...
    afterTest { (_, _) ->
        jclang.Clang.INSTANCE.disposeAll()
    }
    ...
    files.forEach { file ->
        "test ${file.name}" {
            val index = jclang.Clang.INSTANCE.createIndex(false, false)
...
private fun serializeDiagnostics(diagnostics: List<jclang.Diagnostic>): String {
    val sw = StringWriter()
    val out = PrintWriter(sw)
    for (diagnostic in diagnostics) {
        var s = diagnostic.format(jclang.Diagnostic.DisplayOptions.DISPLAY_SOURCE_LOCATION)
