...
fun List<NativeTypeAlias>.generateKotlinFile(outputDirectory: File, packageName: String, declarationRepository: DeclarationRepository) {
	...
	FileSpec.builder(packageName, "TypeAlias")
		.also { fileSpec ->
			asSequence()
				.map { typeAlias -> typeAlias.toSpec(packageName, declarationRepository) }
				.forEach {
					when (it) {
