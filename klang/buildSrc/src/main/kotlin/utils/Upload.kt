 ...
 */
fun initPublishingProcess(file: File, username: String, password: String, automatic: Boolean = false) {
	val authorizationHeader = "UserToken ${Base64.getEncoder().encodeToString("$username:$password".toByteArray())}"

	val deploymentId = uploadToCentral(file, authorizationHeader, automatic)
	...
	do {
		...
		when (status) {
			...
			"VALIDATED" -> {
				if (!automatic) {
					println("[Sonatype Central Upload] Deployment verified!")
					break
				}
 ...
 */
private fun uploadToCentral(file: File, authorizationHeader: String, automatic: Boolean): String {
	println("[Sonatype Central Upload] Uploading to Sonatype Central...")
	val publishingType = when(automatic) {
		true -> "AUTOMATIC"
		else -> "USER_MANAGED"
	}
